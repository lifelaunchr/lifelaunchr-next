...
> Last updated: 2026-04-26 (v1.0.6).

## Version History

| Version | Date | Notes |
|---------|------|-------|
| v1.0.6 | 2026-04-26 | Fix Soar Summary not persisting after drawer close for students (app#108): `onUpdateEntry` in `ListsContent` now optimistically updates `entries` state before awaiting the PATCH call. Previously, `setEntries` was only called inside `updateEntry`'s `res.ok` branch — if the PATCH failed for students, `entries` was never updated and the drawer reinitialised `summaryText` from the stale null value on reopen. DB save from the streaming endpoint was always correct (confirmed by hard refresh); this was a frontend-only state management fix. (`src/app/lists/page.tsx`) |
